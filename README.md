...
¡Claro! Aquí tienes el contenido de la guía técnica completa. Puedes copiar y pegar este texto directamente en un archivo como README.md o GUIA_TECNICA.txt en tu repositorio de GitHub.

Este formato es ideal para la documentación del proyecto y la configuración inicial de tu compañero.

GUIA_TECNICA_PROYECTO_API_REST.txt
🛠️ Configuración Inicial del Entorno de Desarrollo
Esta guía detalla los pasos para poner en marcha la API Rest de Spring Boot (IntelliJ IDEA) utilizando Docker como servidor de base de datos MariaDB local.
...
docker run --name mi-mariadb-local -e MARIADB_ROOT_PASSWORD=mi_clave_secreta -e MARIADB_USER=dam235 -e MARIADB_PASSWORD=D@m235U35. -p 3306:3306 -d mariadb:latest
2. Configuración de la Conexión de la API
Verifica que el archivo application.properties de tu proyecto Spring Boot apunte a esta base de datos local:
...
# ... otras configuraciones (driver, dialecto) ...
3. Ejecución del Script SQL Corregido
Para crear las tablas y datos iniciales (resolviendo el error de AUTO_INCREMENT), conéctate a la consola de MariaDB como root o dam235 (el cual ya tiene permisos GRANT ALL PRIVILEGES).

Comando de Conexión (como dam235):

Bash

docker exec -it mi-mariadb-local mariadb -u dam235 -p
(Contraseña: D@m235U35.)
...
-- 2. CREACIÓN DE LA TABLA USER (CORREGIDA con AUTO_INCREMENT)
DROP TABLE IF EXISTS user;
CREATE TABLE user ( 
    id int(11) NOT NULL AUTO_INCREMENT,
    name varchar(256) CHARACTER SET utf8mb3 COLLATE utf8mb3_spanish_ci NOT NULL, 
